...
export default class PermissionUtils {
  ...
  async defaultGrantPermission(): Promise<boolean> {
    try {
      ...
      Logger.info(TAG,
        `defaultGrantPermission tokenId :${tokenId},atManager:${this.atManager} checkAccessToken  + : ${JSON.stringify(PermissionArray)}`);
      for (let i = 0; i < PermissionArray.length; i++) {
        try {
          let state = await this.atManager.checkAccessToken(tokenId, PermissionArray[i]);
          Logger.info(TAG,
            `defaultGrantPermission  checkAccessToken ${PermissionArray[i]} + : ${JSON.stringify(state)}`);
          if (state !== abilityAccessCtrl.GrantStatus.PERMISSION_GRANTED) {
            pems.push(PermissionArray[i]);
          }
        } catch (error) {
          Logger.error(TAG,
            `defaultGrantPermission  checkAccessToken error ${PermissionArray[i]} error: ${JSON.stringify(error)}`);
        }
      }
      if (pems.length > 0) {
